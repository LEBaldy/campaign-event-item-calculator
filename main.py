...
try:
  ...
  while type(itemspurchased) == str:
    ...
    try:
      ...
      if (hoursremaining < 24 or (hoursremaining == 24 and minutesremaining == 0)): raise GeneratorExit
    except Exception as noninteger:
  ...
  while type(custominput) == str:
    ...
    try:
      if custominput=="none" or custominput=="no":break
      else:custominput = int(custominput)
      if custominput < currentamount:
        
        print("\nInput number must be greater than current items. Please input an integer (whole number) for custom lookup or type \"none\".")
        custominput = "Fail"
    except Exception as noninteger:
  ...
  def zcalc(current, hoursleft, day):
    try:
      if (minutesremaining == 0):
        stdvpredicted = stdv[int(hoursleft - 1)]
      else:
        stdvpredicted = ((stdv[int(math.ceil(hoursleft) - 1)] -stdv[int(math.floor(hoursleft)-1)]) /(math.ceil(hoursleft) - math.floor(hoursleft))) * (hoursleft - math.floor(hoursleft)) + stdv[int(math.floor(hoursleft) - 1)]
    ...
    try:
      if (minutesremaining == 0):
        stdvdone = stdv[int((168 - hoursleft) - 1)]
      else:
        stdvdone = ((stdv[int(math.ceil(168 - hoursleft) - 1)] -stdv[int(math.floor(168 - hoursleft)-1)]) /(math.ceil(168 - hoursleft) - math.floor(168 - hoursleft))) * (hoursleft - math.floor(hoursleft)) + stdv[int(math.floor(168 - hoursleft) - 1)]
    except Exception as stdvrange:
      pass
    ...
    probfor2500=str(round(100*(1-norm.cdf((1800 - currentamount + 100 * day -1774.08 / 168 * hoursleft) / stdvpredicted)),2))+"%"
    ...
    if type(custominput)==str:return (nowprobXlow, nowprobXhigh, zscorezonenow, currprobXlow, currprobXhigh, zscorezonecurr, stdvpredicted, probfor2500, "none")
    else:return (nowprobXlow, nowprobXhigh, zscorezonenow, currprobXlow, currprobXhigh, zscorezonecurr, stdvpredicted, probfor2500, str(round(100*(1-norm.cdf((custominput - currentamount + 100 * (day - 7) -1774.08 / 168 * hoursleft) / stdvpredicted)),2))+"%")
  ...
  def campaigncalc(hours, minutes, current, Purchased, Spent):
    ...
    if zero_time_check:
      ...
      estimateaverage = math.floor(168 * 10.56 + (7 - dayNumber) * 100)
      return (7 - dayNumber, estimateaverage)
  ...
  def output(results, text):
    ...
    if (customprob!="" or prob2500!="") and results[4]>=0:customprob+="\n"

    if(results[4]<0):outputtext=text[0]+text[1][money]+text[2][spant]+"\n\nPredicted Item Count (average rate): "+str(results[1])+" Items\nPredicted range: "+str(math.floor(results[1]-results[3][6]))+"-"+str(math.floor(results[1]+results[3][6]))+" Items\n"+prob2500+customprob+"----------------------------------------------------------\nYou have missed daily rewards this event.\nA current rate will not be given.\n----------------------------------------------------------"
    else:outputtext=text[0]+text[1][money]+text[2][spant]+text[3]+prob2500+customprob+text[4][curr]+text[8][currodd[0]]+text[9][currodd[1]]+text[5][now]+time+text[8][nowodd[0]]+text[9][nowodd[1]]+"\n----------------------------------------------------------"
  ...
  try:
    if zero_time_check:
      default1, purchase, spent, default2, currprob, nowprob, hour, minute, oddsdefault,odds,prob2500,probcustom="\n----------------------------------------------------------\nTime Remaining: "+str(results[0])+" days "+str(hoursremaining - 24 * results[0] - 24)+" hours "+str(minutesremaining)+" minutes\nDaily Loot Remaining: "+str(100 * results[0])+" Items",["","\nTotal items purchased: "+str(amountpurchased)+" Items"],["","\nTotal items spent: "+str(itemspurchased)+" Items"],"\n\nPredicted Item Count (average rate): "+str(results[1])+" Items\nPredicted Item Count (current rate): "+str(results[2])+" Items\nPredicted range: "+str(math.floor(results[1]-results[3][6]))+"-"+str(math.floor(results[1]+results[3][6]))+" Items\n----------------------------------------------------------\n",["You have a "+str(round(results[3][3]*100,3))+"% to be "+str(results[2])+" items or lower by the end of the event.","You have a "+str(round(results[3][4]*100,3))+"% to be "+str(results[2])+" items or higher by the end of the event."],["\nYou had a "+str(round(results[3][0]*100,3))+"% to be "+str(currentamount)+" items or lower at ","\nYou had a "+str(round(results[3][1]*100,3))+"% to be "+str(currentamount)+" items or higher at "],["1 hour",str(192-hoursremaining)+" hours"],["1 minute into event.",str(60-minutesremaining)+" minutes into event."],["","\n~~These odds are lower than "],["","rolling the same number on a dice three times in a row.~~","rolling the same number on a dice four times in a row.~~","getting 3 5* in a 10 pull of heroic scrolls.~~","getting injured by a toilet or finding a four-leaf clover.~~","being struck by lightning.~~","truly being 1 in a million.~~"],"You have a "+results[3][7]+" chance to reach 2500 items for eminent chest.\n", "You have a "+results[3][8]+" chance to reach "+str(custominput)+" items by end of event.\n"

      print(output(results,[default1, purchase, spent, default2, currprob, nowprob, hour, minute, oddsdefault,odds, prob2500, probcustom]))
    else:
      default="\n----------------------------------------------------------\nTime Remaining: "+str(results[0])+" days\nDaily Loot Remaining: "+str(100 * results[0])+" Items\nTotal items purchased: 0 Items\nTotal items spent: 0 Items\n\nPredicted Item Count (average rate): "+str(results[1])+" Items\nPredicted range: "+str(math.floor(results[1]-stdv[168]))+"-"+str(math.floor(results[1]+stdv[168]))+" Items"+str(round(100*(1-norm.cdf((1800 -1774.08 ) / stdv[-1])),2))+"%"+"\n----------------------------------------------------------\nEvent has not started.\nA current rate will not be given.\n----------------------------------------------------------"
      print(default)

  except Exception as err:
    print("An error has occured\n", err,"\nPlease contact me on reddit or discord with the error above.")
except GeneratorExit as gen:
  if type(currentamount)==str or type(itemspurchased)==str:print("\n----------------------------------------------------------\nWhats the point of the calculator if the event is over?\nYou already know the values with 100% certainty.")
